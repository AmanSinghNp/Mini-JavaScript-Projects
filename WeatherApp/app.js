...
/**
 * Get local time for a specific city based on coordinates
 * @param {number} lat - Latitude
 * @param {number} lon - Longitude
 * @returns {Date} Local time for the city
 */
function getCityLocalTime(lat, lon) {
  // Calculate timezone offset based on longitude
  // Longitude changes by 15 degrees per hour (360° / 24 hours)
  // This is an approximation and doesn't account for actual timezone boundaries
  let timezoneOffset = lon / 15;

  // Round to nearest 30-minute interval (some timezones are offset by 30 mins)
  timezoneOffset = Math.round(timezoneOffset * 2) / 2;

  // Clamp to valid timezone range (-12 to +14 hours)
 ...
 * @param {number} lat - City latitude (optional)
 * @param {number} lon - City longitude (optional)
 * @param {boolean} isDay - API is_day parameter (optional)
 * @returns {string} 'light' or 'dark'
 */
function getTimeBasedTheme(lat = null, lon = null, isDay = null) {
  ...
  if (lat !== null && lon !== null) {
    currentTime = getCityLocalTime(lat, lon);
  } else {
...
function updateThemeToggle() {
  ...
  switch (mode) {
    case THEME_MODES.AUTO:
      // Try to get current theme, fallback to coordinate-based calculation
      const currentTheme = getTimeBasedTheme(
        state.current.lat,
        state.current.lon
      );
      themeIcon.textContent = currentTheme === THEME_MODES.LIGHT ? "☀️" : "🌙";
...
function setThemeMode(mode) {
  ...
  if (mode === THEME_MODES.AUTO) {
    startAutoThemeCheck();
    applyTheme(getTimeBasedTheme(state.current.lat, state.current.lon));
  } else {
    stopAutoThemeCheck();
    applyTheme(mode);
  }
...
function startAutoThemeCheck() {
  ...
  state.theme.checkInterval = setInterval(() => {
    if (state.theme.mode === THEME_MODES.AUTO) {
      const newTheme = getTimeBasedTheme(state.current.lat, state.current.lon);
      applyTheme(newTheme);
      updateThemeToggle();
    }
...
function chooseBackground(conditionCode) {
  ...
  return BACKGROUND_IMAGES[iconName] || BACKGROUND_IMAGES.clear;
}
...
function renderWeather(data) {
  ...
  if (
    state.theme.mode === THEME_MODES.AUTO &&
    hourly.is_day?.[0] !== undefined
  ) {
    const newTheme = getTimeBasedTheme(null, null, hourly.is_day[0]);
    applyTheme(newTheme);
    updateThemeToggle();
  }
...
function setHeroBackground(code) {
  ...
  img.onerror = () => {
    ...
    const fallbackGradients = {
      ...
      overcast:
        "linear-gradient(135deg, #636e72 0%, #2d3436 30%, #636e72 100%)",
    ...
    const iconName = ICON_MAP[code] || "clear";
    const fallbackGradient =
      fallbackGradients[iconName] || fallbackGradients.clear;
    hero.style.backgroundImage = fallbackGradient;
  };
  img.src = background;
}
...
function updateDateTime() {
  // Use city's local time if available, otherwise use user's local time
  const cityTime =
    state.current?.lat && state.current?.lon
      ? getCityLocalTime(state.current.lat, state.current.lon)
      : new Date();

  datetime.textContent = cityTime.toLocaleDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  datetime.dateTime = cityTime.toISOString();
}
...
async function selectCity(city) {
  ...
  // Theme will be updated in renderWeather() when API data is available
  // This provides more accurate day/night detection than longitude calculation
